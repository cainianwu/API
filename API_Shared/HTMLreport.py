...
class PATH:
    """ all file PATH meta """
    config_tmp_path = '../API_Shared/template'
...
class ReportTestResult(unittest.TestResult):
    ...
    def __init__(self, suite, stream=sys.stdout):
        ...
        self.past = "../TESTreport/"
        self.annotation = "接口测试案例"
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description = None, filename: str = None, log_path=None):
        super(ReportTestResult, self).__init__()
        if log_path is None:
            log_path = self.past
        else:
...
class htmltsetrunner():

    def reporterll(self,filenmail,rev_runsuit,revtitle,revdescription,):

        filename = "../TESTreport/" + filenmail
        with open(filename, 'wb') as fhtml:
            HTMLTestRunnerCN.HTMLTestReportCN(
            stream=fhtml,
            title=revtitle,
            verbosity=2,
...
#testuin.addTest(s_house("The_editor"))
